...
const ACCOUNT_CURRENCIES = ['GBP', 'EUR', 'USD'] as const;
	...
	},
	{
		...
		icon: (
			<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
				...
				<defs>
					<clipPath id="clip0_854_3578"><rect width="24" height="24" fill="white" /></clipPath>
				</defs>
			</svg>
		),
	},
...
function getTransactionIcon(type: string) {
	switch (type) {
		case "withdrawal": return <ArrowDown className="w-5 h-5 text-[#211E1E] dark:text-white" />;
		case "topup": return <ArrowUp className="w-5 h-5 text-[#211E1E] dark:text-white" />;
		case "transfer": return <ArrowRightLeft className="w-5 h-5 text-[#211E1E] dark:text-white" />;
		case "exchange": return <ExchangeArrowsIcon className="text-[#211E1E] dark:text-white" />;
		default: return null;
	}
}
...
function getMoveFundsButtonStyles(accountId: AccountType, accountColor: string) {
	if (accountId === "pension") {
		return { className: "w-full py-4 rounded-lg mb-4 font-regular text-base flex items-center justify-center gap-2 bg-[#211E1E] text-white dark:bg-white dark:text-black", style: {} };
	}
	return { className: "w-full py-4 rounded-lg mb-4 font-regular text-white text-base flex items-center justify-center gap-2", style: { backgroundColor: accountColor } };
}
...
const AccountDetail: React.FC = () => {
	...
	const handleMoveFunds = () => {
		if (accountId === "pension") {
			...
			navigate("/move-funds", { state: { sourceAccount: accountId, destinationAccount: "currentAccount" } });
		}
	};

	const actionsWithHandlers = config.actions?.map(action => {
		if (action.label === 'Exchange') {
			return { ...action, onClick: () => navigate('/exchange', { state: { fromCurrency: selectedCurrency } }) };
		}
		return action;
	});
	...
	const formatCurrencyBalance = (balance: number, currCode: string) => {
		...
		const parts = balance.toLocaleString("en-GB", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).split(".");
		return { symbol: info.symbol, wholePart: parts[0], decimalPart: parts[1] };
	};
		...
		<div className="min-h-screen bg-[#F3F3F3] dark:bg-black text-foreground max-w-[480px] mx-auto flex flex-col pb-24">
			<div className="px-4 py-6 flex flex-col flex-1">
				...
				<div>
					...
					{/* Currency Tabs - only for currentAccount, only GBP/EUR/USD */}
					{accountId === 'currentAccount' && (
						<>
					...
					{accountId !== 'currentAccount' && (
						<div className="rounded-lg bg-white dark:bg-[#211E1E] flex items-center justify-between pt-2 pb-2 pl-2 pr-5 mb-4">
							<div className="flex items-center gap-2">
								<div className="w-12 h-12 flex items-center justify-center bg-[#000] dark:bg-[#000] rounded-[4px]" style={{ color: account.color }}>
									<span className="text-2xl">{account.icon}</span>
								</div>
								<span className="text-xl font-normal text-foreground">Balance</span>
							</div>
							<div className="text-lg ml-2 font-normal text-foreground">{formatCurrency(account.balance)}</div>
						</div>
					)}
					...
					{config.moveFundsButton && (() => {
						...
						return (
							<button onClick={handleMoveFunds} className={buttonStyles.className} style={buttonStyles.style}>
								<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
						...
						{filteredTransactions.length === 0 ? (
							...
							<div>
								{filteredTransactions.map((tr) => (
									<div key={tr.id} className="flex items-center gap-4 py-[12px] border-b border-[#E5E5EA] dark:border-[#2C2C2E] hover:bg-[#E5E5EA] dark:hover:bg-[#1C1C1E]/30 transition-colors cursor-pointer">
